...
while(<STDIN>) {
    ...
    foreach $w (split (" ",$trans)) {
        $w =~ tr:A-Z:a-z:;  # Lowercase everything to match the processing of other datasets.
        ...
        $w =~ s:\*(.*)\*:\1:g;             # Mispronounced words are enclosed in asterisks; we don't care
        if ($w ne "!exclamation-point") {  # ! indicates unusual emphasis; we don't care
            $w =~ s:!::g;
        }
        if ($w ne ":colon") {              # : indicates a lengthened sound; we don't care
            $w =~ s:\:::g;
        }

        # Words we don't want to print
