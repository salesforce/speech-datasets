...
while(<CSV>) {
  ...
  # speaker comes from commonvoice --> uttId comes from commonvoice
  $spkr = "{commonvoice}$spkr";
  if ("$gender" eq "female") {
    $gender = "f";
  } else {
  ...
  # get rid of all puncts besides apostrophes
  $text =~ s/[^\w\s']|//g;
  $text =~ s/(\s)'/$1/g;
  $text =~ s/'(\s)/$1/g;
...
# Use utt2spk to generate spk2utt
if (system(
