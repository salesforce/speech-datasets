...
if [ $# != 1 ] && [ $# != 2 ]; then
  ...
  log "Usage: $0 /path/to/SWBD [/path/to/SWBD_DOC]"
  exit 1;
fi 

SWBD_DIR=$1
dir=data/local/swbd1
mkdir -p $dir

sph2pipe=$MAIN_ROOT/tools/sph2pipe_v2.5/sph2pipe
if [ ! -x $sph2pipe ]; then
  log "Could not execute the sph2pipe program at $sph2pipe"
  exit 1
fi

# Option A: SWBD dictionary file check
dict=${SWBD_DIR}/swb_ms98_transcriptions/sw-ms98-dict.text
if [ ! -f $dict ]; then
  log  "SWBD dictionary file $dict does not exist"
  exit 1
fi
...
n=$(wc -l < $dir/sph.flist)
if [ "${n}" != 2435 ] && [ "${n}" != 2438 ]; then
  log "Warning: expected 2435 or 2438 data data files, found $n"
fi
...
# LM vocabs.
awk '{ 
       name=substr($1,1,6); gsub("^sw","{swbd}0",name); side=substr($1,7,1);
       stime=$2; etime=$3;
       printf("%s-%s_%06.0f-%06.0f",
              name, side, int(100*stime+0.5), int(100*etime+0.5));
       for(i=4;i<=NF;i++) printf(" %s", $i); printf "\n"
}' ${SWBD_DIR}/swb_ms98_transcriptions/*/*/*-trans.text  > $dir/transcripts1.txt
...
# speech to somone; map [VOCALIZED-NOISE], [LAUGHTER], and [NOISE] to
# <noise>, which we give the phone NSN. There is also a silence phone, SIL.
# **NOTE: modified the pattern matches to make them case insensitive
cat $dir/transcripts1.txt \
  | perl -ane 's:(\s|^)\[SILENCE\](\s|$):$2:gi;
               s:\[VOCALIZED-NOISE\]|\[LAUGHTER\]|\[NOISE\]:<noise>:gi;
               s/<B_ASIDE>//gi; 
               s/<E_ASIDE>//gi; 
               print;' \
...
#{swbd}02001-A_000098-001156 {swbd}02001-A 0.98 11.56
awk '{ 
       segment=$1;
       split(segment,S,"[_-]");
...
# update utterance names from sw02001 to {swbd}02001
sed -e 's?.*/sw?{swbd}?' -e 's?.sph??' $dir/sph.flist | paste - $dir/sph.flist \
  > $dir/sph.scp
...
# this file reco2file_and_channel maps recording-id (e.g. {swbd}02001-A)
# to the file name {swbd}02001 and the A, e.g.
# {swbd}02001-A {swbd}02001 A
...
awk '{spk=substr($1,1,13); print $1 " " spk}' $dir/segments > $dir/utt2spk \
  || exit 1;
...
for f in spk2utt utt2spk wav.scp text segments reco2file_and_channel; do
  cp $dir/$f $dest/$f || exit 1;
done

if [ $# == 2 ]; then # fix speaker IDs
  find $2 -name conv.tab > $dir/conv.tab
  local/swbd1_fix_speakerid.pl `cat $dir/conv.tab` $dest
  utils/utt2spk_to_spk2utt.pl $dest/utt2spk.new > $dest/spk2utt.new
  # patch files
  for f in spk2utt utt2spk text segments; do
    cp $dest/$f $dest/$f.old || exit 1;
    cp $dest/$f.new $dest/$f || exit 1;
  done
  rm $dir/conv.tab
fi 
rm -r $dir

log "Making training & development split"
log "$(utils/subset_data_dir.sh --first ${dest} 4000 ${dest}_dev) for dev set" # 5hr 6min
n=$(($(wc -l < ${dest}/segments) - 4000))
log "$(utils/subset_data_dir.sh --last ${dest} ${n} ${dest}_train0) for train set"
log "Removing duplicates from train set: $(utils/remove_dup_utts.sh 300 ${dest}_train0 ${dest}_train)" # 286hr
rm -r ${dest} ${dest}_train0

log "Switchboard-1 data preparation succeeded."
