...
if [ $# -ne 2 ]; then
  log "Usage: $0 <speech-dir> <transcription-dir>"
  log "See comments in the script for more details"
  exit 1
fi
...
if [ $(wc -l < $dest/wav.scp) -ne 80 ]; then
  log "$0: error: expected 80 lines in wav.scp, got $(wc -l < $dest/wav.scp)"
  exit 1;
fi

log "Data preparation and formatting completed for Eval 2000 (but not MFCC extraction)"
