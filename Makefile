...
# 1.5.0 and later do not work with ESPNet
TORCH_VERSION := 1.4.0
...
CONDA_PYTORCH := pytorch=$(TORCH_VERSION) cudatoolkit=$(CUDA_VERSION) -c pytorch
else
CUDA_VERSION :=
CONDA_PYTORCH := pytorch=$(TORCH_VERSION) cpuonly -c pytorch
endif
...
ifneq ($(TORCH_VERSION),)
	...
	tools/install_anaconda.sh $(PYTHON_VERSION) "$(CONDA)" tools/venv $(VENV_NAME) . "$(CONDA_PYKALDI)"
endif
...
ifneq ($(strip $(CUDA_VERSION)),)
	source tools/venv/etc/profile.d/conda.sh && conda deactivate && conda activate $(VENV_NAME) && python tools/check_install.py
else
