...
if [ $# -ne 3 ]; then
    log "Usage: $0 [--remove-archive] <data-base> <url-base> <corpus-part>"
    log "e.g.: $0 /export/a15/vpanayotov/data www.openslr.org/resources/11 dev-clean"
    log "With --remove-archive it will remove the archive after successfully un-tarring it."
    log "<corpus-part> can be one of: dev-clean, test-clean, dev-other, test-other,"
    log "          train-clean-100, train-clean-360, train-other-500."
    exit 1
fi
...
if ! $part_ok; then
    log "$0: expected <corpus-part> to be one of $list, but got '$part'"
    exit 1
fi

if [ -z "$url" ]; then
    log "$0: empty URL base."
    exit 1
fi

if [ -f $data/LibriSpeech/$part/.complete ]; then
    log "$0: data part $part was already successfully extracted, nothing to do."
    exit 0
fi
...
if [ -f $data/$part.tar.gz ]; then
    ...
    if ! $size_ok; then
        log "$0: removing existing file $data/$part.tar.gz because its size in bytes $size"
        log "does not equal the size of one of the archives."
        rm $data/$part.tar.gz
    else
        log "$data/$part.tar.gz exists and appears to be complete."
    fi
fi
...
if [ ! -f $data/$part.tar.gz ]; then
    ...
    log "$0: downloading data from $full_url.  This may take some time, please be patient."

    if ! wget -P $data --no-check-certificate $full_url; then
        log "$0: error executing wget $full_url"
        exit 1
    fi
fi

if ! tar -C $data -xvzf $data/$part.tar.gz; then
    log "$0: error un-tarring archive $data/$part.tar.gz"
    exit 1
fi

touch $data/LibriSpeech/$part/.complete

log "$0: Successfully downloaded and un-tarred $data/$part.tar.gz"

if $remove_archive; then
  log "$0: removing $data/$part.tar.gz file since --remove-archive option was supplied."
  rm $data/$part.tar.gz
fi
