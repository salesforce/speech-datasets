...
from speech_datasets.utils.types import str2bool, str_or_none
...
def tokenize(
    ...
    token_type: str,
    space_symbol: str,
    non_linguistic_symbols: Optional[Union[str, Iterable[str]]],
    ...
    words_and_counts = list(sorted(counter.items(), key=lambda x: x[1], reverse=True))
    ...
    for nvocab, (w, c) in enumerate(words_and_counts, 1):
        ...
        if max_nvocab is not None and nvocab >= max_nvocab:
            nvocab = min(nvocab, max(max_nvocab, 0))
            break

    # Add user-specified symbols to the vocabulary
    words_and_counts = words_and_counts[:nvocab]
    ...
    for word, _ in words_and_counts:
        fout.write(word + "\n")

    logger.info(f"OOV rate = {(total_count - invocab_count) / total_count * 100} %")
