...
from speech_datasets.utils.readers import HDF5SignalRateFile
...
class SpeedPerturbation(object):
    ...
    and sox-speed just to resample the input, i.e pitch and tempo are both changed.
    ...
    def __init__(
        ...
        if utt2ratio is not None:
            ...
            with open(utt2ratio, "r") as f:
                for line in f:
                    utt, ratio = line.rstrip().split(None, maxsplit=1)
                    ratio = float(ratio)
    ...
    def __call__(self, x, uttid=None, train=True):
        ...
        y = resampy.resample(x, ratio, 1, filter=self.res_type)
...
class NoiseInjection(object):
    """Add either isotropic noise, or a noise sample saved to a file."""
    def __init__(
        self,
        lower=-20,
        upper=-5,
        ...
        if utt2noise is not None:
            ...
            if filetype == "list":
                with open(utt2noise, "r") as f:
                    for line in f:
                        utt, filename = line.rstrip().split(None, maxsplit=1)
                        signal, rate = soundfile.read(filename, dtype="int16")
                        # Load all files in memory
                        self.utt2noise[utt] = (signal, rate)

            elif filetype == "hdf5":
                self.utt2noise = HDF5SignalRateFile(utt2noise, "r")
            else:
    ...
    def __call__(self, x, uttid=None, train=True):
        ...
        if self.utt2noise is not None:
            ...
            if uttid is not None and uttid in self.utt2noise:
...
class RIRConvolve(object):
    def __init__(self, utt2rir, filetype="list"):
        ...
        if filetype == "list":
            ...
            self.utt2rir = HDF5SignalRateFile(utt2rir, "r")
        else:
