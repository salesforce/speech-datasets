...
def time_warp(x, max_time_warp=80, inplace=False, mode="PIL"):
    ...
    if mode == "PIL":
        ...
        import torch
        return sparse_time_warp.time_warp(torch.from_numpy(x), window).numpy()
    else:
        raise NotImplementedError(f"Unknown resize mode: {mode}. "
                                  f"Choose one from (PIL, sparse_image_warp).")
...
def spec_augment(x, resize_mode="PIL", max_time_warp=80, max_freq_width=27,
                 n_freq_mask=2, max_time_width=100, n_time_mask=2, inplace=True,
