...
if [ $# -ne 1 ]; then
  echo "Usage: $0 [--no-feats] [--no-text] [--no-scp] [--no-spk-sort] [--scp-name <scp_name>] <data-dir>"
...
if [ -f $data/images.scp ]; then
  cmd=${cmd/--no-scp/}  # remove --no-scp if supplied
...
if [ -f $data/segments ] && [ ! -f $data/$scp_name ]; then
  echo "$0: in directory $data, segments file exists but no $scp_name"
  exit 1;
fi


if [ ! -f $data/$scp_name ] && ! $no_scp; then
  echo "$0: no such file $data/$scp_name (if this is by design, specify --no-scp, or change the value of --scp-name)"
  exit 1;
fi

if [ -f $data/$scp_name ]; then
  check_sorted_and_uniq $data/$scp_name

  if grep -E -q '^\S+\s+~' $data/$scp_name; then
    ...
    # tilde expansion.
    echo "$0: Please do not use tilde (~) in your $scp_name."
    exit 1;
  fi
  ...
  if [ -f $data/segments ]; then
    ...
    awk '{print $1}' $data/$scp_name > $tmpdir/recordings.wav
    if ! cmp -s $tmpdir/recordings{,.wav}; then
      echo "$0: Error: in $data, recording-ids extracted from segments and $scp_name"
      echo "$0: differ, partial diff is:"
    ...
    cat $data/$scp_name | awk '{print $1}' > $tmpdir/utts.wav
    if ! cmp -s $tmpdir/utts{,.wav}; then
      echo "$0: Error: in $data, utterance lists extracted from utt2spk and $scp_name"
      echo "$0: differ, partial diff is:"
...
if [ -f $data/reco2dur ]; then
  ...
  if [ -f $tmpdir/recordings ]; then
    ...
    if ! cmp -s $tmpdir/{utts,recordings.reco2dur}; then
      echo "$0: Error: in $data, recording-ids extracted from $scp_name and reco2dur file"
      echo "$0: differ, partial diff is:"
